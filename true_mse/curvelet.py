...
wedgeMap = []
scale_num = 0
for shape in FDCT.shapes:
    wedge_num = 0
    for wedge in shape:
        wedgeMap.append((scale_num, wedge_num, np.std(c_struct[scale_num][wedge_num])))
        wedge_num += 1
    scale_num += 1
...
def get_fdct_struct(c_str, x):
    ...
    for w_i, w in enumerate(wedgeMap):
        i = w[0]
        j = w[1]
        c_copy[i][j] = pywt.threshold(c_copy[i][j], x[w_i], mode='soft')
    return c_copy
...
def naive_min(c_str):
    ...
    for i, c_scale in enumerate(c_copy):
        for j, wedge in enumerate(c_scale):
            count += 1
            print(count, end='\r')
            wedge_std = np.std(wedge)

            orig = wedge.copy()
            min_wedge = wedge.copy()
            mse = np.inf
            t = 0
            for delta in np.arange(0, 1, 0.1):
                ...
                # Threshold the wedge
                c_copy[i][j] = pywt.threshold(wedge, t, mode='soft')
                ...
                if (x_mse >= mse):
                    break
                min_wedge = c_copy[i][j].copy()
                mse = x_mse
                c_copy[i][j] = orig.copy()
            ts.append(t)
            c_copy[i][j] = min_wedge.copy()

    return ts

# Define the search space
# space = list(map(lambda x: Real(0, x[2]), wedgeMap))
