...
file_pattern = r"^(?P<name>.*)_(?P<shader>pt|lt|lt_opt|lt_imp_dist|lt_imp_dist_cam|lt_imp_dist_cam_rt|ref)_(?P<buffer_type>diff_acc|spec_acc|caus_acc|rad_acc)_(?P<sequence_number>\d+)(?:\.(?P<extension>\w+))?$"
shader_pattern = r"^(pt|lt|lt_opt|lt_imp_dist|lt_imp_dist_cam|lt_imp_dist_cam_rt|ref)$"
ref_name = 'ref'
...
def compare_images(ref_data, noisy_image):
    ...
    # Compare images
    # score_ref = compute_score(ref_data, ref_data, imageLoaderStr)
